...
namespace _6270PersonalAttempt
{
    class LineRatesStandard
    {
        ...
        public decimal getBaseRate2()
        {
            decimal amount = 0.19M;
        ...
        public decimal getFinishTypeUpcharge2()
        {
            decimal amount = 0M;
            if (lineItem.FinishType == FinishTypeChoices.matte)
            {
        ...
        public decimal DeliveryRateUpcharge()
        {
            
            decimal amount = 0m;
            if (lineItem.DeliveryType == DeliveryTypeChoices.nextDay) { 
                if (lineItem.Quantity <= 60) amount = 1M;
                else if (lineItem.Quantity >60 ) amount = 2M;              
            }
            else if (lineItem.DeliveryType == DeliveryTypeChoices.oneHour)
            {
                if (lineItem.Quantity <= 60) amount = 2M;
                else if (lineItem.Quantity > 60) amount = 2.5M;
            }
            return amount;
        ...
        {
            return getBaseRate() + getFinishTypeUpcharge() + DeliveryRateUpcharge();
        }

        public decimal getLineRate2()
        {
            return getBaseRate2() + getFinishTypeUpcharge2() + DeliveryRateUpcharge();
        }
    }
}
